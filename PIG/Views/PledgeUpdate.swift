...
import SwiftUI
import FirebaseAuth
//The view which allows a user to update a pledge and track activity for it
struct PledgeUpdate: View {
    ...
    var body: some View {
        VStack{
            //If pledge completed and the screen needs to go back, go back to the parents view to this view
            if (goBack) {
                ...
                Button(action: {
                    //Show different things depending on which pledge is selected for update
                    if(allowedToTrack(pledgeInQuestion: pledgeToUpdate)){
                        if(pledgeToUpdate.description.contains("Walk to work")){
                            ...
                            //Just add 1 and update if no advanced tracking implemented yet
                        ...
                        //User has already tracked today
                    ...
                    if value == true {
                        //When pledge marked as completed, perform the updates
                        updatePledge(pledgeToUpdate: pledgeToUpdate, daysCompleted: pledgeToUpdate.daysCompleted, durationInDays: pledgeToUpdate.durationInDays)
                        toastShow = true
    ...
    //Manually update pledge in firestore
