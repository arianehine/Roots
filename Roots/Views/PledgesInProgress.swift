...
//Displays all of the users pledges, those in progress and those completed. Shows an icon for each like a badge/award
struct PledgesInProgress: View {
    ...
    //Sets the notification reminder for a pledge if the toggle changes to true (on)
    func setNotifReminder(pledge: Pledge){
        ...
        // show this notification 24h from now
    ...
    //Request permissions to notify the user of pledge reminders
    ...
    //Initialise variables we need in this class
    func initVars(){
...
//To represent a pledge which is empty
